...
function _cached(key, ttlMs, fn) {
  const hit = _wCache.get(key), now = Date.now();
  if (hit && hit.expires > now) return hit.data;
  return Promise.resolve().then(fn).then(data => {
    _wCache.set(key, { expires: now + ttlMs, data }); return data;
  });
}
async function geocode(name) {
  const url = new URL("https://geocoding-api.open-meteo.com/v1/search");
  url.searchParams.set("name", name); url.searchParams.set("count", "1");
  url.searchParams.set("language", "en"); url.searchParams.set("format", "json");
  const r = await fetch(url); if (!r.ok) throw new Error(`Geocoding failed: ${r.status}`);
  const j = await r.json(); const loc = j?.results?.[0];
  if (!loc) throw new Error(`Could not find location: ${name}`);
  return { name: `${loc.name}${loc.admin1 ? ", " + loc.admin1 : ""}${loc.country ? ", " + loc.country : ""}`,
           lat: loc.latitude, lon: loc.longitude, tz: loc.timezone };
}
...
async function fetchForecast({ lat, lon, tz }) {
  ...
  url.searchParams.set("hourly","temperature_2m,apparent_temperature,precipitation_probability,precipitation,rain,snowfall,weather_code,wind_speed_10m,wind_gusts_10m");
  url.searchParams.set("daily","weather_code,temperature_2m_max,temperature_2m_min,precipitation_probability_max,precipitation_sum,sunrise,sunset,wind_speed_10m_max");
  url.searchParams.set("forecast_days","7");
  const r = await fetch(url); if (!r.ok) throw new Error(`Forecast failed: ${r.status}`);
  return await r.json();
}
const forecastCached = (loc) => _cached(`fc:${loc.lat},${loc.lon}`, 10*60*1000, () => fetchForecast(loc));

function pickHourIndex(hourly, localISO) {
  if (!localISO || !hourly?.time?.length) return -1;
...
export function toolsJobs(factory) {
  const addJob = factory({
    ...
    impl: async (a) => {
      ...
      const row = { id: newId("J"), created_at: new Date().toISOString(),
        company: a.company, role: a.role, url: a.url || "", source: a.source || "",
        status: a.status || "wishlist", notes: a.notes || "" };
      rows.push(row); await writeJson("jobs", rows);
  ...
  const listJobs = factory({
    ...
    schema: z.object({ status: z.enum(["wishlist","applied","interview","offer","rejected"]).optional() }),
    impl: async ({ status }) => {
      const rows = await readJson("jobs");
  ...
  const updateJob = factory({
    ...
    schema: z.object({ id: z.string(), status: z.enum(["wishlist","applied","interview","offer","rejected"]).optional(), notes: z.string().optional() }),
    impl: async ({ id, status, notes }) => {
      const rows = await readJson("jobs");
      const i = rows.findIndex(r => r.id === id);
      if (i < 0) return { updated: false, message: `Not found: ${id}` };
      if (status) rows[i].status = status;
      if (notes) rows[i].notes = notes;
    ...
    impl: async () => {
      ...
      for (const key of Object.keys(scanned || {})) {
        ...
        if (ex) {
          ...
          rows.push({
            id: newId("J"), created_at: new Date().toISOString(),
            company, role, url: "", source: "Gmail",
            status: it.status || "applied", notes: it.subject || "", last_update: it.last_update || ""
          });
          upserts++;
        }
      }
...
export function toolsSubs(factory) {
  const addSubscription = factory({
    ...
    impl: async (a) => {
      ...
      const row = { id: newId("S"), created_at: new Date().toISOString(),
        service: a.service, status: a.status || "active",
        price: a.price || "", renewal_or_end: a.renewal_or_end || "", notes: a.notes || "" };
      rows.push(row); await writeJson("subs", rows);
  ...
  const updateSubscription = factory({
    ...
    schema: z.object({
      id: z.string(), status: z.enum(["trial","active","canceled"]).optional(),
      notes: z.string().optional(), renewal_or_end: z.string().optional(), price: z.string().optional()
    }),
    impl: async ({ id, status, notes, renewal_or_end, price }) => {
      ...
      if (status) rows[i].status = status;
      if (notes) rows[i].notes = notes;
      if (renewal_or_end) rows[i].renewal_or_end = renewal_or_end;
      if (price) rows[i].price = price;
      rows[i].updated_at = new Date().toISOString();
  ...
  const importSubscriptionsFromGmail = factory({
    ...
    impl: async () => {
      ...
      for (const it of items) {
        ...
        rows.push({
          id: newId("S"), created_at: new Date().toISOString(),
          service: it.service || "Unknown", status: it.status || "active",
          price: it.price || "", renewal_or_end: it.renewal_or_end || "", notes: it.subject || ""
        });
        index.set(key, true); upserts++;
      }
      await writeJson("subs", rows);
      return { imported: true, upserts, total: rows.length };
    }
...
export function toolsWeather(factory) {
  const getWeather = factory({
    name: "getWeather",
    description: "Get current/forecast weather for a location. Optional 'when' like 'today 6pm' or 'tomorrow morning'.",
    ...
    impl: async ({ location, when }) => {
      ...
      if (when) {
        ...
        let hh = null, mm = "00";
        if (m1) {
          hh = parseInt(m1[1], 10); mm = m1[2] || "00";
          const ap = m1[3].toLowerCase();
          if (ap === "pm" && hh !== 12) hh += 12;
          if (ap === "am" && hh === 12) hh = 0;
        } else if (m2) { hh = parseInt(m2[1], 10); mm = m2[2]; }
        if (hh !== null) {
          const now = new Date();
      ...
      return {
        location: place.name, timezone: place.tz, current: fc.current || null,
        hourly: {
          time: fc.hourly?.time || [],
...
export function toolsSentiment(factory) {
  const POS = new Set(["good","great","excellent","love","happy","amazing","fantastic","positive","wonderful","awesome"]);
  const NEG = new Set(["bad","terrible","awful","hate","sad","poor","horrible","negative","angry","disappointing"]);

  function score(text) {
    const toks = (text || "").toLowerCase().match(/[a-z']+/g) || [];
    let s = 0; for (const t of toks) { if (POS.has(t)) s++; if (NEG.has(t)) s--; }
  ...
  const analyzeSentiment = factory({
    ...
    impl: async ({ text }) => {
      const { label, score } = score(text);
      const freq = Object.create(null);
      for (const w of (text.toLowerCase().match(/[a-z]{5,}/g) || [])) freq[w] = (freq[w] || 0) + 1;
      const phrases = Object.entries(freq).sort((a,b)=>b[1]-a[1]).slice(0,8).map(([w,c])=>({ phrase:w, count:c }));
      return { label, score, phrases, chars: text.length };
    }
  });
...
export function toolsResume(factory, getContext) {
  ...
  function topSnippets(text, question, k = 3) {
    ...
    const scores = chunks.map((c,i)=>{
      let s=0; const lc=c.toLowerCase(); for(const term of q) if (lc.includes(term)) s++;
      return { i, s, text:c };
    });
    return scores.sort((a,b)=>b.s-a.s).slice(0,k).map(x=>({ index:x.i, score:x.s, text:x.text }));
  }
  ...
  const resumeStatus = factory({
    ...
    schema: z.object({}),
    impl: async () => {
      const { sid } = getContext(); const text = await readResume(sid);
  ...
  const resumeAsk = factory({
    ...
    impl: async ({ question }) => {
      const { sid } = getContext(); const text = await readResume(sid);
      if (!text) return { has_resume:false, message:"No resume uploaded for this session." };
  ...
  const resumeImprove = factory({
    name: "resumeImprove",
    description: "Provide top snippets for improvement suggestions (skills, impact metrics, clarity).",
    schema: z.object({ focus: z.string().optional() }),
    impl: async ({ focus }) => {
      const { sid } = getContext(); const text = await readResume(sid);
      if (!text) return { has_resume:false, message:"No resume uploaded for this session." };
      const snippets = topSnippets(text, focus || "experience results impact clarity", 6);
      return { has_resume:true, snippets, note:"Suggest concrete edits with examples and metrics." };
    }
  });
